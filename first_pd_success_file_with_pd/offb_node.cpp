...
void local_pos_cb(const geometry_msgs::PoseStamped::ConstPtr& msg){
    local_pos = *msg;
    std::cout << "local_pos.pose.position.z:" << local_pos.pose.position.z << std::endl;
...
int main(int argc, char **argv)
{
    ...
    double roll,pitch,yaw;
     //将roll, pitch,yas赋给四元数后，可以得到转换后的四元数
        ...
        if( current_state.mode != "OFFBOARD" &&
            ...
            {       
                ...
                case 0: 
                    ...
                    Vdes_z = Kpz*(5-local_pos.pose.position.z);
                    attitude.thrust = 0.7+ Kpvz*(Vdes_z-local_vel.twist.linear.z);
                    if(attitude.thrust > 0.8)
                        {attitude.thrust = 0.8;}
                    if(attitude.thrust < 0.05)
                        {attitude.thrust = 0.05;}

                    ROS_INFO("thrust1:%f",attitude.thrust);
                    ROS_INFO("aixis_z:%f",local_pos.pose.position.z);
                    ROS_INFO("local_vel.twist.linear.z == %f",local_vel.twist.linear.z);
                    if (local_pos.pose.position.z > 4.8 && local_pos.pose.position.z < 5.2)
                    {   

                        if (sametimes < 1000)
                        sametimes++;
                        else if (sametimes > 1000 && sametimes < 7000)
                        {
                            sametimes = 0;
                            step = 1;
                        attitude.orientation.x = 0;
                    ...
                    break;
                case 1:   //在空中一米处飞到（2,5,0）的位置
                    // while(1){
                        
                        Vrx=Kpx*(2-local_pos.pose.position.x);
                        plustheta_x = Kpx_theta*(Vrx-local_vel.twist.linear.x);
                        
                        Vry=Kpy*(2-local_pos.pose.position.y);
                        plustheta_y = Kpy_theta*(Vry-local_vel.twist.linear.y);
                        ...
                        //ROS_INFO("the theta of roll:%f",euler.Pitch);
                        ROS_INFO("ROLL:%f",roll);
                        ROS_INFO("PITCH:%f",pitch);
                        ROS_INFO("the theta of quaternion:%f",attitude.orientation.x);
                        ROS_INFO("the theta of quaternion:%f",attitude.orientation.y);
                        ROS_INFO("the theta of quaternion:%f",attitude.orientation.z);
                        ROS_INFO("the theta of quaternion:%f",attitude.orientation.w);
                        // ROS_INFO("the thrust is:%f",attitude.thrust);
                        ROS_INFO("the position of x is: %f",local_pos.pose.position.x);
                        ROS_INFO("the position of y is: %f",local_pos.pose.position.y);
                        ROS_INFO("the position of z is: %f",local_pos.pose.position.z);
